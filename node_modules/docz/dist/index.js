...
const BasePlayground = loadable(() => Promise.resolve(require('./chunk2.js')));
const Playground = props => typeof window !== 'undefined' ? (React.createElement(React.Suspense, { fallback: null },
...
const AsyncComponent = defaultProps => {
    ...
    const { as: Comp, getInitialProps } = defaultProps, props = tslib_1.__rest(defaultProps, ["as", "getInitialProps"]);
...
const loadRoute = (path, imports, components) => {
    ...
    const fn = async () => {
        const importFn = fp.get(path, imports);
        const { default: Component, getInitialProps } = await importFn();
        const ExportedComponent = props => (React.createElement(AsyncComponent, Object.assign({}, props, { as: Component || 'div', getInitialProps: getInitialProps })));
        return ExportedComponent;
    };
    return loadable(fn, { fallback: React.createElement(Loading, null) });
};
const AsyncRoute = defaultProps => {
    const { asyncComponent, path, entry } = defaultProps, routeProps = tslib_1.__rest(defaultProps, ["asyncComponent", "path", "entry"]);
...
const Link = React.forwardRef((defaultProps, ref) => {
    const props = fp.omit(['activeClassName', 'partiallyActive'], defaultProps);
    const isActive = React.useCallback(({ isCurrent }) => {
        return isCurrent ? { className: `${props.className} active` } : {};
    }, [props.className]);
    return React.createElement(router.Link, Object.assign({}, props, { getProps: isActive, ref: ref }));
});
...
const getPropType = (prop) => {
    const propName = fp.get('name', prop.flowType || prop.type);
    if (!propName)
        return null;
    const isEnum = propName.startsWith('"') || propName === 'enum';
    const name = capitalize(isEnum ? 'enum' : propName);
    const value = fp.get('type.value', prop);
    if (!name)
        return null;
...
const Props = ({ of: component }) => {
    const components = __chunk_1.useComponents();
    const { props: stateProps } = React.useContext(__chunk_1.doczState.context);
    const PropsComponent = components.props;
    const filename = fp.get('__filemeta.filename', component);
    const componentName = component.displayName || component.name;
    const found = stateProps &&
        stateProps.length > 0 &&
        stateProps.find(item => item.key === filename);
    const value = fp.get('value', found) || [];
    const firstDefinition = fp.first(value);
    const definition = value.find((i) => i.displayName === componentName);
    const props = fp.get('props', definition || firstDefinition);
    if (!props)
        return null;
    if (!PropsComponent)
        return null;
    return React.createElement(PropsComponent, { props: props, getPropType: getPropType });
};
...
const Routes = ({ imports }) => {
    const components = __chunk_1.useComponents();
    const { entries } = React.useContext(__chunk_1.doczState.context);
    const NotFound = components.notFound;
    const history = React.useMemo(() => router.createHistory(window), []);
    React.useEffect(() => {
        history.listen(goToHash);
    }, []);
    return (React.createElement(react.MDXProvider, { components: components },
        React.createElement(router.LocationProvider, { history: history },
            React.createElement(router.Router, null,
                ...
                entries &&
                    entries.map(({ key: path, value: entry }) => {
                        ...
                        return (React.createElement(AsyncRoute, Object.assign({}, props, { entry: entry, key: entry.id, path: entry.route, asyncComponent: component })));
                    })))));
};
...
function theme(themeConfig, transform = c => c) {
    return WrappedComponent => {
        const Theme = React.memo(props => {
            ...
            const { db, children, wrapper: Wrapper = React.Fragment } = props;
            const initial = Object.assign({}, db, { themeConfig, transform, linkComponent });
            return (React.createElement(__chunk_1.doczState.Provider, { initial: initial },
