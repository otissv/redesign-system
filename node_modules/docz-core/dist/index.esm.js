...
import { readJsonSync, ensureDir, remove as remove$1, outputJSONSync, emptyDir, pathExists, copy, readFileSync, readJSONSync, pathExistsSync, existsSync } from 'fs-extra';
import { get, set } from 'env-dot-prop';
import humanize from 'humanize-string';
import titleize from 'titleize';
import { get as get$1, isFunction, omit, merge, isEmpty, entries, contains, prop, propEq } from 'lodash/fp';
...
const getEnv = (val, defaultValue = null) => get(val, defaultValue, { parse: true });
const getInitialTitle = (pkg) => {
    const name = get$1('name', pkg) || 'MyDoc';
    return titleize(humanize(name.replace(/^@.*\//, '')));
};
const getInitialDescription = (pkg) => get$1('description', pkg) || 'My awesome app using docz';
const setArgs = (yargs) => {
    const pkg = readJsonSync(appPackageJson, { throws: false });
    return yargs
        .option('base', {
        type: 'string',
...
const populateNodePath = () => {
    ...
    set('node.path', get('node.path', '')
        .split(delimiter)
        .filter((folder) => folder && !isAbsolute(folder))
        .map((folder) => resolve(root, folder))
        .join(delimiter));
};
const configDotEnv = () => {
...
const createId = (file) => createHash('md5')
    .update(file)
    .digest('hex');
...
class Entry {
    ...
    getRoute(parsed, base) {
        const parsedRoute = get$1('route', parsed);
...
class Plugin {
    ...
    static propsOfPlugins(plugins) {
        return prop => plugins && plugins.length > 0
            ? plugins.map(p => get$1(prop, p)).filter(Boolean)
            : [];
    }
    ...
    async getMap(config) {
        ...
        const createEntry = async (file) => {
            try {
                ...
                const { settings } = entry, rest = __rest(entry, ["settings"]);
...
class DataServer {
    ...
    setState(key, val) {
        const prev = get$1(key, this.getState());
    ...
    async writeDbFile() {
        outputJSONSync(db, this.mapToObject(this.state), { spaces: 2 });
    }
    mapToObject(map) {
...
const getBaseConfig = (argv, custom) => {
    const initial = omit(toOmit, argv);
...
const parseConfig = async (argv, custom) => {
    ...
    const config = argv.config
        ? loadFrom(resolve(argv.config), defaultConfig)
        : load('docz', defaultConfig);
...
const createConfig = (args, env) => async (hooks) => {
    ...
    config.output
        ...
        .devtoolModuleFilenameTemplate((info) => resolve(info.resourcePath).replace(/\\/g, '/'));
    /**
     * entries
     */
    config
...
const hasCiEnvVar = () => get('ci', false, { parse: true });
...
const updateEntries = (entries) => async (p) => {
    const prev = get$1('entries', p.getState());
...
const state = (entries, config) => {
    ...
    const files = Array.isArray(config.files)
        ? config.files.map(filePath => join(src, filePath))
        : join(src, config.files);
...
const update = async (params, initial, { config }) => {
    const next = config
        ? loadFrom(resolve(config), initial, true, true)
...
const digest = (str) => createHash('md5')
    .update(str)
    .digest('hex');
const cacheFilepath = join(cache, 'propsParser.json');
const readCacheFile = () => readJSONSync(cacheFilepath, { throws: false });
function checkFilesOnCache(files) {
    ...
    return files.filter(filepath => {
        ...
        const found = get$1(normalized, cache);
...
function getPropsOnCache() {
    ...
    return Object.entries(cache).map(([key, value]) => ({
        key,
        value: get$1('props', value),
    }));
}
const mergeWithCache = (cache, props) => {
    const keys = props.map(prop('key'));
    return cache.filter(item => !contains(item.key, keys)).concat(props);
};
const removeFromCache = (filepath) => {
    const cache = readCacheFile();
    outputJSONSync(cacheFilepath, omit(filepath, cache));
};
const getInitialFilesMap = () => {
    ...
    if (isEmpty(cache))
        return new Map();
    const map = new Map();
    entries(cache).forEach(([filepath]) => {
...
const parseFiles = (files, config, tsconfig) => {
    ...
    const compilerOptions = get$1('options', getTSConfigFile(tsconfig));
...
const getPattern = (config) => {
    ...
    const src = relative(root, source);
    return ignore
        .map(entry => `!**/${entry}`)
        .concat([
...
const change = (p, config) => async (filepath) => {
    const prev = get$1('props', p.getState());
