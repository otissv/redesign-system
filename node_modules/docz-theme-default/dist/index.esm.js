...
`;
const Logo = ({ showBg }) => {
    const { base, title, linkComponent: Link, themeConfig: { logo }, } = useConfig();
    if (!Link)
        return null;
    return (createElement(Wrapper$4, { showBg: showBg },
        createElement(Link, { to: typeof base === 'string' ? base : '/' }, logo ? (createElement(LogoImg, { src: logo.src, width: logo.width, alt: title })) : (createElement(LogoText, null, title)))));
};
